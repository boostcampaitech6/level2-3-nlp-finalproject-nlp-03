...
class Raptor:
    ...
    def GMM_cluster(self, embeddings: np.ndarray, threshold: float, random_state: int = 0):
        """Cluster embeddings using a Gaussian Mixture Model (GMM) based on a probability
        threshold.
    ...
    def embed_cluster_texts(self, texts):
        """Embeds a list of texts and clusters them, returning a DataFrame with texts, their
    ...
    def embed_cluster_summarize_texts(
        ...
        assignments for easier processing, and then summarizes the content within each cluster.
        ...
        # Summarization
        template = """해당 문서는 청년정책 신청과 관련된 문서입니다.

        청년정책은 청년의 경제적 자립, 사회적 참여, 문화적 창조 활동을 지원하여 사회 구성원으로서 성장할 수 있도록 돕는 정책입니다.

        해당 문서는 정책 소개, 신청 절차 혹은 용어 풀이 등 청년정책 신청을 도와주는 내용들로 구성되어 있습니다.
    ...
    def recursive_embed_cluster_summarize(
        ...
        """Recursively embeds, clusters, and summarizes texts up to a specified level or until the
        number of unique clusters becomes 1, storing the results at each level.
